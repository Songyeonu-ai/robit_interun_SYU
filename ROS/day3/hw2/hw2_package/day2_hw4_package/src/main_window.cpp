...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent), ui(new Ui::MainWindowDesign)
{
  ...
  qnode = new QNode();
  node_ = std::make_shared<rclcpp::Node>("hw4_node"); //qnode 참고, 새로운 노드(rclcpp:Node 클래스 시용 위함)
  // node_->declare_parameter("link_1", 100); //초깃값 설정
  // node_->declare_parameter("link_2", 80);
  // node_->declare_parameter("link_3", 50);
  // node_->declare_parameter("joint_degrees", 100);
  link_1 = node_->get_parameter("link_1").as_int();
  link_2 = node_->get_parameter("link_2").as_int();
  link_3 = node_->get_parameter("link_3").as_int();
  joint_degree = node_->get_parameter("joint_degrees").as_int();
...
void MainWindow::paintEvent(QPaintEvent *e)
{
    QPainter painter(this);
        ...
        painter.drawLine(0, 0, link_1, 0);     //팔 그리기

        // 2번째 관절
        QPen pen2(Qt::red, 7);
        painter.setPen(pen2);
        painter.translate(link_1, 0);          // 첫 번째 팔 끝
        painter.rotate(joint_degree);       // 관절 2 회전
        painter.drawLine(0, 0, link_2, 0);

        QPen pen3(Qt::blue, 4);
        painter.setPen(pen3);
        painter.translate(link_2, 0);
        painter.rotate(joint_degree);
        painter.drawLine(0, 0, link_3, 0);

        QPen pen4(Qt::green, 4); //그리퍼 부분 표시
        painter.setPen(pen4);
        painter.translate(link_3,0);
